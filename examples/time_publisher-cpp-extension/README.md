# Build the SDK and Sample C++ Extension

## Introduction

In this example, you will build the SDK as well as a small C++ program that will publish the current time to a UDP port and learn about how to deploy and do basic debugging on the player. We will start by building and testing on the development host, then building the program for the target player.

## Prerequisites

Make sure you have read through the main [README.md](../../README.md) file.
...
Many of the command blocks that follow in the file can be executed as "Notebook" cells when using the VS Code extension "RunME".  They can also, of course, be copied and pasted. To facilitate reproducibility, defining a environment variable is convenient to help orient working directories and will be used in the shell blocks.  Use of this environment variable is optional, but very helpful.

```bash
# make sure the current working directory is the examples/time-publisher-cpp-extension directory
...
**Open** and inspect the [`make-extension-lvm`](../common-scripts/make-extension-lvm) and [`make-extension-ubi`](../common-scripts/make-extension-ubi). In particular you may need to set the `name` to match your extension name.
